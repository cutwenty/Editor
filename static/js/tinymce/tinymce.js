...
(function () {
    ...
    var getClosestCell = function (getYAxisValue, isTargetCorner, table, x, y) {
      var cells = descendants$1(Element$$1.fromDom(table), 'td,th').map(function (e) {
        return e.dom();
      });
    ...
    var trimBr = function (first, block) {
      CaretFinder.positionIn(first, block.dom()).each(function (position) {
        ...
        if (NodeType.isBr(node)) {
          remove$2(Element$$1.fromDom(node));
        }
      });
    };
    ...
    var findParentInsertPoint = function (toBlock, block) {
      var parents$$1 = parents(block, function (elm) {
        return eq(elm, toBlock);
      });
      return Option.from(parents$$1[parents$$1.length - 2]);
    };
    var getInsertionPoint = function (fromBlock, toBlock) {
      if (contains$3(toBlock, fromBlock)) {
        return parent(fromBlock).bind(function (parent$$1) {
          return eq(parent$$1, toBlock) ? Option.some(fromBlock) : findParentInsertPoint(toBlock, fromBlock);
        });
      } else {
        return Option.none();
      }
    ...
    var mergeBlockInto = function (rootNode, fromBlock, toBlock) {
      if (Empty.isEmpty(toBlock)) {
        ...
        return getInsertionPoint(fromBlock, toBlock).fold(function () {
          ...
          var position = CaretFinder.prevPosition(toBlock.dom(), CaretPosition$1.before(target.dom()));
          each(children_1, function (node) {
            before(target, node);
          });
          removeEmptyRoot(rootNode, fromBlock);
